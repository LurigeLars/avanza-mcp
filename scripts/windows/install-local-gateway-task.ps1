...
Register-ScheduledTask -TaskName $TaskName -Action $action -Trigger @($logonTrigger, $watchdogTrigger) -Principal $principal -Settings $settings -Description "Runs the loopback-only model-optimized Avanza MCP gateway on 127.0.0.1:8766 without a visible terminal window." -Force | Out-Null

$listener = Get-NetTCPConnection -LocalPort 8766 -State Listen -ErrorAction SilentlyContinue | Select-Object -First 1
if ($listener) {
    Write-Host "Task '$TaskName' registered but not started because port 8766 is already in use by PID $($listener.OwningProcess)."
    exit 0
}
...
Write-Host "Optimized MCP URL: http://127.0.0.1:8766/mcp"
