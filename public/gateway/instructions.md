...
- Raw certificate/warrant filters default to compact filterOptions: small vocabularies remain, the huge underlyingInstruments list is omitted, and category trees are reduced to top-level entries. Resolve underlyings with search_instruments. Request filter_options_mode="full" only when complete upstream filter metadata is genuinely required; use "none" for minimal output.
